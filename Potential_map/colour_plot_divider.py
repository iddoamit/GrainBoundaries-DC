...
params = {"Number of rows (n)": 15,
          ...
          "Length distribution address": None,
          "Doping distribution address": None, 
          "Applied voltage": 1,
          ".raw file": path_here+"Si 15x15 ND 1e16 L 15um T 300K NT 4e11 Point Contacts 2.raw",
...
def SimParser(indx, Vinmap_defined, v_node=params['Voltage names'], n=params["Number of rows (n)"], m=params["Number of columns (m)"], raw_file=params[".raw file"], SimDir=params["Sim Dir"], SimFile=params["Sim file"]):
    ...
    for i in range(n):
        for j in range(3*m-3):
            ...
            if j == 3*m-4:
                ...
                try:
                    V_row_map[i, j] = LT.get_data(str(f'V(x{j:02.0f}y{i:02.0f})'))[indx] - LT.get_data(str(f'V(x{j+1:02.0f}y{i:02.0f})'))[indx]
                except: 
                ...
                try:
                    ...
                    if params["Multiple sources locs"]==None:
                        for v in v_node:
                            try:
                                V_row_map[i, j] = LT.get_data(str(f'V({v})'))[indx] - LT.get_data(str(f'V(x{j+1:02.0f}y{i:02.0f})'))[indx]
                                V_in_map[i] = v
                                break
                            except:
                                V_row_map[i, j] = 0
                        ...
                        V_row_map[i, j] = LT.get_data(str(f'V({Vinmap_defined[i]})'))[indx] - LT.get_data(str(f'V(x{j+1:02.0f}y{i:02.0f})'))[indx]
                ...
                # print(str(f'V(x{j:02.0f}y{i:02.0f})-V(x{j+1:02.0f}y{i:02.0f})'))
                row_map[i, j] = LT.get_data(str(f'I(Rx{j}y{i})'))[indx]
                V_row_map[i,j] = LT.get_data(str(f'V(x{j:02.0f}y{i:02.0f})'))[indx] - LT.get_data(str(f'V(x{j+1:02.0f}y{i:02.0f})'))[indx]
        ...
        for j in range(0,3*m-2,3):
            ...
            if j == 3*m-3:
                if V_ground_map[i] == 1 and V_ground_map[i+1] == 1:
                    ...
                    V_col_map[i, j-1] = -LT.get_data(str(f'V(x{j:02.0f}y{i+1:02.0f})'))[indx]
                    ...
                    V_col_map[i, j-1] = LT.get_data(str(f'V(x{j:02.0f}y{i:02.0f})'))[indx] - LT.get_data(str(f'V(x{j:02.0f}y{i+1:02.0f})'))[indx]
                ...
                if V_in_map[i] != 0 and V_in_map[i+1]!=0:
                    ...
                    V_col_map[i, j] = LT.get_data(str(f'V({V_in_map[i]})'))[indx] - LT.get_data(str(f'V(x{j:02.0f}y{i+1:02.0f})'))[indx]
                    print(f"NOTE: Voltage {V_in_map[i]} above node x{j:02.0f}y{i+1:02.0f}.")
                elif V_in_map[i+1] != 0:
                    V_col_map[i, j] = LT.get_data(str(f'V(x{j:02.0f}y{i:02.0f})'))[indx] - LT.get_data(str(f'V({V_in_map[i+1]})'))[indx]
                    print(f"NOTE: Voltage {V_in_map[i+1]} below node x{j:02.0f}y{i:02.0f}.")
                else:
                    V_col_map[i, j] = LT.get_data(str(f'V(x{j:02.0f}y{i:02.0f})'))[indx] - LT.get_data(str(f'V(x{j:02.0f}y{i+1:02.0f})'))[indx]
    ...
    plt.colorbar(V)
    plt.figure(3)
    P = plt.imshow(masked_Imap*masked_Vmap, origin="lower", extent=(0, 1e-3, 1e-3, 0), cmap='Blues_r')
    plt.colorbar(P)
    plt.show()
